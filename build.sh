...
docker build -t pgtunebuild:el9 -f Dockerfile-el9 .
...
docker run --rm --user $(id -u):$(id -g) -v $PWD:$PWD -w $PWD pgtunebuild:el9 rpmbuild -D "%buildnumber $BUILD_NUMBER" -D "%_topdir $PWD/buildroot" -ba pgtune.spec
